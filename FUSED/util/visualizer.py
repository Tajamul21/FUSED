...
class COCOVisualizer():
    ...
    def visualize(self, img, tgt, caption=None, dpi=120, savedir=None, show_in_console=True):
        ...
        """
        plt.figure(dpi=dpi, figsize=(13.0,10.0))
        ...
        if savedir is not None:
            if caption is None:
                savename = '{}/{}.png'.format(savedir, str(tgt['filename']))
            else:
                savename = '{}/{}-{}-{}.png'.format(savedir, caption, str(tgt['filename']), str(datetime.datetime.now()).replace(' ', '-'))
    ...
    def addtgt(self, tgt):
        ...
        #BDD
        # Cityscapes (Car only)
        # id2name = {0:'person', 1:'car'}
        # id2name = {0:'car'}
        id2name = {0:'person', 1:'car', 2:'train', 3:'rider', 4:'truck', 5:'motorcycle', 6:'bicycle', 7:'bus'}
        color_dict = {
                      'car': 'green',
                      'person': 'red',
                      'train': 'yellow',
...
#            import pdb; pdb.set_trace()
            ...
            # color.append((1,0,0))
            try:
                color.append(color_dict[id2name[int(tgt['box_label'][i])]])
            except:
                import ipdb; ipdb.set_trace()
        ...
        p = PatchCollection(polygons, facecolor='none', linewidths=0)
        ax.add_collection(p)
        p = PatchCollection(polygons, facecolor='none', edgecolors=color, linewidths=2)
        # p = PatchCollection(polygons, facecolor='none', edgecolors=[0,1,0], linewidths=3)
        ax.add_collection(p)
        ...
        #         # ax.text(bbox_x, bbox_y, _string, color='black', bbox={'facecolor': 'red', 'alpha': 0.6, 'pad': 1})
